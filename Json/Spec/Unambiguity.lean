...
/-! ## How much a derivation consumes

Every production leaves less text than it was given, and a value leaves strictly less, which is
what lets one induction on the length of the text carry the whole family below.
-/

private theorem token_length {t : Char} {s r : List Char} (h : Token t s r) :
...
private theorem digits_length {s : List Char} {v n : Nat} {r : List Char} (h : Digits s v n r) :
    r.length < s.length := by
  induction h with
  | last => simp
  | cons _ _ ih => simp only [List.length_cons]; omega

private theorem int_length {s : List Char} {v : Nat} {r : List Char} (h : Int' s v r) :
    r.length < s.length := by
  cases h with
  | zero => simp
  | digits _ hd => exact digits_length hd

private theorem frac_length {s : List Char} {f n : Nat} {r : List Char} (h : Frac s f n r) :
    r.length ≤ s.length := by
  cases h with
  | absent => exact Nat.le_refl _
  | present hd => have := digits_length hd; simp only [List.length_cons]; omega

private theorem exp_length {s : List Char} {e : Int} {r : List Char} (h : Exp s e r) :
    r.length ≤ s.length := by
  cases h with
  | absent => exact Nat.le_refl _
...
private theorem sign_length {s : List Char} {b : Bool} {r : List Char} (h : Sign s b r) :
    r.length ≤ s.length := by
  cases h with
  | absent => exact Nat.le_refl _
  | minus => simp

private theorem num_length {s : List Char} {n : Number} {r : List Char} (h : Num s n r) :
    r.length < s.length := by
  cases h with
  | mk hs hi hf he =>
...
private theorem hex4_length {s : List Char} {v : Nat} {r : List Char} (h : Hex4 s v r) :
    r.length + 4 = s.length := by
  cases h with
  | mk => simp

private theorem ch_length {s : List Char} {c : Char} {r : List Char} (h : Ch s c r) :
    r.length < s.length := by
  cases h with
...
private theorem chars_length {s : List Char} {cs r : List Char} (h : Chars s cs r) :
    r.length ≤ s.length := by
  induction h with
  | nil => exact Nat.le_refl _
  | cons hch _ ih => have := ch_length hch; omega

private theorem str_length {s : List Char} {v : String} {r : List Char} (h : Str s v r) :
    r.length < s.length := by
  cases h with
  | mk hcs =>
...
private theorem value_length {s : List Char} {j : Json} {r : List Char} (h : Value s j r) :
    r.length < s.length := (family_length s.length s (Nat.le_refl _)).2.2.1 h

private theorem elements_length {s : List Char} {vs : List Json} {r : List Char}
    (h : Elements s vs r) : r.length ≤ s.length :=
  (family_length s.length s (Nat.le_refl _)).2.2.2.1 h

private theorem member_length {s : List Char} {m : String × Json} {r : List Char}
