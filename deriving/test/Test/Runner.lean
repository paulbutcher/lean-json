...
/-- As in the root package's runner: the `plausible` tactic reports through warnings, which are
errors here, so properties are checked at run time and their binders named by hand. -/
