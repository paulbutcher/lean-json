...
private def valueOf (n : Nat) : Json :=
  match n % 4 with
  | 0 => .null
  | 1 => .num (Number.ofNat n)
  | 2 => .str s!"v{n}"
  | _ => .arr #[.num 1, .obj #[("k", .bool true)]]

/-- Whatever is put at a path is what is found there. -/
abbrev setThenGet : Prop :=
  NamedBinder "n" <| ∀ n : Nat,
    (match sample.set? (pathOf n) (valueOf n) with
      | .ok j => j.get? (pathOf n) == some (valueOf n)
      | .error _ => false) = true

/-- Putting back what was there changes nothing. -/
abbrev setWhatIsThere : Prop :=
  NamedBinder "n" <| ∀ n : Nat,
    (match sample.get? (pathOf n) with
      | some v => (sample.set? (pathOf n) v).toOption == some sample
      | none => true) = true
...
def all : Array TestCase :=
  lookup ++ update ++ removal ++ typed ++ #[
    property "what is set is what is got" setThenGet,
    property "setting what is already there changes nothing" setWhatIsThere,
